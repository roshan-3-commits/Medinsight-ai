    ...
    <div id="report-detail-view" className="space-y-6 max-w-6xl mx-auto pb-16">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3.5 py-2 bg-[#0d7a68] hover:bg-[#0a6352] text-white rounded-lg text-xs font-semibold transition-colors shadow-sm cursor-pointer disabled:opacity-50"
          ...
          <button
            ...
            onClick={onPrint}
            className="flex items-center gap-2 px-3.5 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold transition-colors shadow-sm cursor-pointer"
          >
          ...
          <button
            id="btn-new-upload"
            onClick={onNewUpload}
            className="flex items-center gap-2 px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-semibold transition-colors shadow-sm cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {onViewPatientProfile && (
                <button
                  ...
                  className="px-2.5 py-1 rounded bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-[11px] flex items-center gap-1 transition-colors"
          ...
          <div className="p-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 text-xs">
            <div className="space-y-1">
              ...
              <span className="text-sm font-bold text-slate-900 block">{patientInfo.name}</span>
            </div>
            <div className="space-y-1">
              <span className="text-slate-500 font-medium block">Age</span>
              <span className="text-sm font-bold text-slate-900 block">{patientInfo.age} yrs</span>
            </div>
            <div className="space-y-1">
              <span className="text-slate-500 font-medium block">Gender</span>
              <span className="text-sm font-bold text-slate-900 block">{patientInfo.gender}</span>
            </div>
            <div className="space-y-1">
              <span className="text-slate-500 font-medium block">Report Type</span>
              <span className="text-sm font-bold text-slate-900 block truncate" title={patientInfo.reportType}>
                {patientInfo.reportType}
              </span>
            </div>
            <div className="space-y-1">
              <span className="text-slate-500 font-medium block">Report Date</span>
              <span className="text-sm font-bold text-slate-900 block">{patientInfo.reportDate}</span>
            </div>
          </div>

          {/* Secondary Facility Info */}
          <div className="px-5 py-2.5 bg-slate-50 border-t border-slate-100 flex flex-wrap items-center justify-between text-[11px] text-slate-600 gap-3">
            <div className="flex items-center gap-1.5">
              <Hospital className="w-3.5 h-3.5 text-slate-400" />
              <span>Pathology Center: <strong className="text-slate-800">{patientInfo.labName || 'Apex Diagnostic Labs'}</strong></span>
            </div>
            <div className="flex items-center gap-1.5">
              <Stethoscope className="w-3.5 h-3.5 text-slate-400" />
              <span>Attending Physician: <strong className="text-slate-800">{patientInfo.referringDoctor || 'Dr. A. Verma, MD'}</strong></span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm flex flex-col justify-between space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Biomarker Health Index</span>
            <span className="text-[11px] font-semibold text-slate-400 font-mono">{tests.length} Parameters</span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative w-16 h-16 rounded-full border-4 border-slate-100 flex items-center justify-center shrink-0">
              <div 
                ...
                style={{ opacity: healthIndexPercentage / 100 }}
              />
              <span className="text-base font-extrabold text-slate-900">{healthIndexPercentage}%</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs pt-1">
            <div className="p-2 rounded-lg bg-emerald-50 border border-emerald-200">
              <span className="text-[10px] text-emerald-700 font-semibold block uppercase">Target Safe</span>
              <span className="font-bold text-emerald-900 text-sm">{normalTests} Normal</span>
            </div>
            <div className="p-2 rounded-lg bg-amber-50 border border-amber-200">
              <span className="text-[10px] text-amber-700 font-semibold block uppercase">Action Needed</span>
              <span className="font-bold text-amber-900 text-sm">{summary.abnormalCount} Out of Range</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div 
        ...
        className={`rounded-xl border p-5 shadow-sm space-y-4 transition-all ${
          abnormalTests.length > 0 
            ? 'bg-gradient-to-br from-amber-50/40 via-white to-rose-50/30 border-amber-200/90' 
            : 'bg-gradient-to-br from-emerald-50/40 via-white to-emerald-50/20 border-emerald-200'
        }`}
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200/70">
          <div className="flex items-center gap-2.5">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 shadow-2xs ${
              abnormalTests.length > 0 ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'
            }`}>
              {abnormalTests.length > 0 ? (
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono uppercase px-2 py-0.5 rounded font-bold bg-slate-900 text-white">
                  Executive Triage
                </span>
              </div>
              <p className="text-xs text-slate-500">
        ...
        {abnormalTests.length > 0 ? (
            ...
            {abnormalTests.map((test) => {
              ...
              return (
                <div
                  ...
                  className="p-3.5 rounded-xl bg-white border border-slate-200/90 hover:border-amber-400 hover:shadow-md transition-all duration-150 cursor-pointer flex flex-col justify-between space-y-2.5 group"
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <div className="mt-1 flex items-baseline gap-1.5">
                        ...
                        <span className="text-xs text-slate-500 font-normal">{test.unit}</span>
                    ...
                    <div className="shrink-0">
                      {test.status === 'critical' ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-extrabold bg-rose-100 text-rose-800 border border-rose-300 ring-1 ring-rose-400/30">
                          <AlertTriangle className="w-3 h-3 text-rose-700 animate-pulse" />
                          <span>CRITICAL</span>
                        </span>
                      ) : test.status === 'high' ? (
                  ...
                  {varianceLabel && (
                    <div className="inline-flex items-center gap-1.5 text-[10px] font-mono font-semibold px-2 py-0.5 rounded bg-slate-100 text-slate-700 w-fit">
                  ...
                  <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px]">
                    ...
                    <span className="text-emerald-700 group-hover:text-emerald-800 font-semibold flex items-center gap-1">
          ...
          <div className="p-4 rounded-xl bg-white border border-emerald-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
            ...
            <div className="flex flex-wrap gap-1.5 text-[11px] font-medium text-emerald-800">
              <span className="px-2 py-1 rounded bg-emerald-50 border border-emerald-200">Complete Blood Count: Optimal</span>
              <span className="px-2 py-1 rounded bg-emerald-50 border border-emerald-200">Metabolic Panel: Safe</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden space-y-0">
        <div className="p-5 border-b border-slate-200 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 rounded-full bg-slate-900 text-white flex items-center justify-center font-bold text-xs">
              4
            </div>
            <div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 text-xs">
            ...
            <div className="relative w-44 sm:w-52">
              ...
              <input
                ...
                className="w-full pl-8 pr-2.5 py-1 text-xs bg-slate-50 border border-slate-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-emerald-500 text-slate-800"
              />
            </div>
            ...
            <div className="flex items-center bg-slate-100 rounded-lg p-0.5 border border-slate-200">
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-md font-semibold transition-colors ${
                  filterStatus === 'abnormal' ? 'bg-amber-100 text-amber-900' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-md font-semibold transition-colors ${
                  filterStatus === 'normal' ? 'bg-emerald-100 text-emerald-900' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                Normal ({normalTests})
              </button>
        ...
        <div className="px-5 py-2.5 bg-slate-50 border-b border-slate-100 flex items-center gap-2 overflow-x-auto text-xs">
          <span className="text-slate-400 font-semibold text-[11px] shrink-0">Panels:</span>
          <button
            onClick={() => setFilterCategory('all')}
            className={`px-2.5 py-0.5 rounded-full text-xs font-semibold shrink-0 transition-colors ${
              filterCategory === 'all' ? 'bg-slate-900 text-white' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'
            }`}
          >
            All Panels
          </button>
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-2.5 py-0.5 rounded-full text-xs font-semibold shrink-0 transition-colors ${
                  isSelected ? 'bg-slate-900 text-white' : 'bg-white border border-slate-200 text-slate-700 hover:bg-slate-100'
                }`}
              >
                {cat} ({count})
              </button>
                    ...
                    <React.Fragment key={test.id}>
                      <tr
                        ...
                        className={`hover:bg-slate-50/90 transition-colors ${
                          test.status !== 'normal' ? 'bg-amber-50/20' : ''
                        }`}
                      >
                        {/* Test Name & Category */}
                        ...
                        {/* Measured Result */}
                        <td className="py-3.5 px-4">
                          <div className="font-bold text-slate-900 text-sm font-mono flex items-baseline gap-1">
                        ...
                        <td className="py-3.5 px-4">
                          <div className="w-full space-y-1">
                            <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden flex border border-slate-200 relative shadow-2xs">
                              {/* Low segment */}
                              <div className="w-1/4 bg-amber-200 border-r border-amber-300" title="Low zone" />
                              {/* Normal target safe zone segment */}
                              <div className="w-2/4 bg-emerald-200 border-r border-emerald-300" title="Target healthy zone" />
                              {/* High segment */}
                              <div className="w-1/4 bg-rose-200" title="High zone" />
                              
                              {/* Needle Indicator Pin */}
                              <div
                                className="absolute top-0 bottom-0 w-2.5 -ml-1 rounded-full bg-slate-900 ring-2 ring-white shadow-sm"
                            ...
                            <div className="flex justify-between text-[9px] text-slate-400 font-mono px-0.5">
                              ...
                              <span className="text-emerald-700 font-semibold">Normal Zone</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
          <div className="flex items-center gap-2 pb-3 border-b border-slate-200">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900">Overall Summary</h3>
              <p className="text-[11px] text-slate-500">Categorized diagnostic findings</p>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-200 space-y-1 text-xs">
            ...
            <p className="font-bold text-slate-900">{summary.overallHealthStatus}</p>
            <p className="text-slate-600 text-xs leading-relaxed">{summary.headline}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
          <div className="flex items-center gap-2 pb-3 border-b border-slate-200">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">Dietary adjustments & lifestyle modifications</p>
            </div>
          </div>
